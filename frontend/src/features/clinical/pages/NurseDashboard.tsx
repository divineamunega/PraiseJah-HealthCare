        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <h3 className="text-[10px] font-bold text-on-surface-variant uppercase tracking-[0.2em] border-b border-white/5 pb-2">
                Cardiovascular
              </h3>
...
const Nurse_Dashboard = () => {
  ...
  const stats = {
    waitingForVitals: activeVisits.filter(
      (v) => v.queueEntry?.status === "WAITING_FOR_VITALS",
    ).length,
    readyForDoctor: activeVisits.filter(
      (v) => v.queueEntry?.status === "READY_FOR_DOCTOR",
    ).length,
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-end justify-between">
        ...
        <div className="flex gap-3">
          <div
            className={`flex items-center gap-3 bg-surface-container-low px-4 py-2 border transition-all ${isFetching ? "border-clinical-blue shadow-[0_0_15px_rgba(0,183,255,0.1)]" : "border-white/5"}`}
          >
        ...
        {[
          {
            ...
            count: stats.waitingForVitals,
            color: "text-yellow-400",
            icon: Clock,
          },
          {
            label: "READY FOR DOCTOR",
            count: stats.readyForDoctor,
            color: "text-green-400",
            icon: CheckCircle2,
          },
          {
            label: "IN CLINICAL REVIEW",
          ...
          <div
            key={stat.label}
            className="bg-surface-container-low p-6 border-none relative overflow-hidden group"
          >
            ...
            <p className="text-4xl font-bold text-white data-value mt-2">
              {isLoading ? "..." : stat.count}
            </p>
            <div
              className={`absolute bottom-0 left-0 h-0.5 bg-current opacity-20 transition-all group-hover:opacity-50 ${stat.color}`}
              style={{ width: "100%" }}
            />
          </div>
        ))}
            ...
            <div className="space-y-3 relative">
              ...
              {isLoading ? (
                <div className="p-20 text-center border border-dashed border-white/5 bg-surface-container-low/30">
                  <Loader2
                    size={24}
                    className="animate-spin mx-auto text-clinical-blue mb-4"
                  />
                  <p className="text-[10px] text-on-surface-variant uppercase tracking-widest">
                    Synchronizing Clinical Feed...
                  </p>
                </div>
              ) : activeVisits.length > 0 ? (
                activeVisits.map((visit) => {
                  const isWaiting =
                    visit.queueEntry?.status === "WAITING_FOR_VITALS";
                  return (
                    <div
                      key={visit.id}
                      className={`bg-surface-container-low p-4 flex items-center justify-between group transition-all border border-white/5 border-l-2 ${
                        isWaiting
                          ? "border-l-yellow-400 hover:bg-yellow-400/[0.02]"
                          : "border-l-clinical-blue hover:bg-clinical-blue/[0.02]"
                      }`}
                    >
                      <div className="flex gap-6 items-center">
                        <div
                          className={`data-value text-[10px] w-16 font-bold ${isWaiting ? "text-yellow-400" : "text-clinical-blue"}`}
                        >
                ...
                <div className="p-20 text-center border border-dashed border-white/5 bg-surface-container-low/10">
                  ...
                  <p className="text-[10px] text-on-surface-variant uppercase tracking-[0.2em]">
                    Queue Clear — Clinical Station Standby
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2">
              {isLoadingRecent ? (
                ...
                recentVitals.map((v: any) => (
                  <div
                    key={v.id}
                    className="bg-surface-container-low/50 p-4 border border-white/5 flex items-center justify-between group"
                  >
                    <div className="flex gap-4 items-center">
                      ...
                      <div>
                        <p className="text-xs font-bold text-white uppercase">
                          {v.visit?.patient?.firstName}{" "}
                          {v.visit?.patient?.lastName}
                        </p>
        ...
        <div className="space-y-6">
          <div className="bg-surface-container-low border border-white/5 p-6 space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end">
                ...
                <span className="text-xl font-bold text-white data-value">
                  92%
                </span>
              </div>
          ...
          <div className="bg-clinical-blue/5 border border-clinical-blue/10 p-4">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-[10px] font-bold text-clinical-blue uppercase tracking-widest">
                Auto-Queue Active
              </span>
            </div>
            <p className="text-[10px] text-on-surface-variant leading-relaxed">
              Patients automatically advance to Doctor Review upon successful
              vital capture.
            </p>
          </div>
        </div>
