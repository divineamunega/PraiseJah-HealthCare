...
const SUPER_ADMIN_RolePermissions = () => {
  ...
  const roles = [
    {
      name: 'SUPER_ADMIN',
      description: 'Full system access including configuration and database management',
  ...
  const permissionModules = [
    {
      ...
      permissions: [
        ...
        { action: 'View Users', roles: ['SUPER_ADMIN', 'ADMIN', 'DOCTOR', 'NURSE', 'SECRETARY'] },
      ],
    },
    {
      ...
      icon: Activity,
      permissions: [
        { action: 'Create Patients', roles: ['SUPER_ADMIN', 'ADMIN', 'SECRETARY'] },
        { action: 'Edit Patients', roles: ['SUPER_ADMIN', 'ADMIN', 'SECRETARY'] },
        { action: 'Delete Patients', roles: ['SUPER_ADMIN', 'ADMIN'] },
        { action: 'View Patients', roles: ['SUPER_ADMIN', 'ADMIN', 'DOCTOR', 'NURSE', 'SECRETARY'] },
      ],
    },
    {
      ...
      permissions: [
        ...
        { action: 'View Notes', roles: ['SUPER_ADMIN', 'ADMIN', 'DOCTOR', 'NURSE'] },
        { action: 'Prescribe Medication', roles: ['SUPER_ADMIN', 'ADMIN', 'DOCTOR'] },
      ],
    },
    {
      ...
      icon: Thermometer,
      permissions: [
        { action: 'Manage Queue', roles: ['SUPER_ADMIN', 'ADMIN', 'NURSE', 'SECRETARY'] },
        { action: 'Record Vitals', roles: ['SUPER_ADMIN', 'ADMIN', 'NURSE'] },
        { action: 'View Queue', roles: ['SUPER_ADMIN', 'ADMIN', 'DOCTOR', 'NURSE', 'SECRETARY'] },
        { action: 'Assign to Doctor', roles: ['SUPER_ADMIN', 'ADMIN', 'NURSE'] },
      ],
    },
    {
    ...
    },
    {
      ...
      icon: Calendar,
      permissions: [
        { action: 'Schedule Appointments', roles: ['SUPER_ADMIN', 'ADMIN', 'SECRETARY'] },
        { action: 'View Schedule', roles: ['SUPER_ADMIN', 'ADMIN', 'DOCTOR', 'NURSE', 'SECRETARY'] },
        { action: 'Cancel Appointments', roles: ['SUPER_ADMIN', 'ADMIN', 'SECRETARY'] },
        { action: 'Check-in Patients', roles: ['SUPER_ADMIN', 'ADMIN', 'SECRETARY'] },
      ],
    },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-end justify-between">
        <div>
          <p className="mono-label text-clinical-blue mb-1 uppercase">Access Control</p>
          <h1 className="text-3xl font-bold text-white tracking-tighter">Role Permissions</h1>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {roles.map((role) => (
          <div key={role.name} className={`bg-surface-container-low p-6 space-y-4 border-l-2 ${role.borderColor}`}>
            <div>
            ...
            <div className="space-y-2">
              {role.permissions.slice(0, 3).map((perm) => (
                <div key={perm} className="flex items-center gap-2 text-xs text-on-surface-variant">
                  <CheckCircle size={10} className={role.color} />
                  <span>{perm}</span>
                </div>
              ))}
              {role.permissions.length > 3 && (
                <p className="text-xs text-on-surface-variant">+{role.permissions.length - 3} more</p>
              )}
            </div>
          </div>
        ))}
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-white/5">
                      <th className="px-4 py-2 mono-label text-on-surface-variant text-[10px]">ACTION</th>
                      {roles.map((role) => (
                        <th key={role.name} className="px-4 py-2 mono-label text-on-surface-variant text-[10px] text-center">{role.name}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {module.permissions.map((perm) => (
                      <tr key={perm.action} className="border-b border-white/5 last:border-0">
                        <td className="px-4 py-2 text-xs text-white">{perm.action}</td>
                        {roles.map((role) => (
                          <td key={role.name} className="px-4 py-2 text-center">
                            {hasPermission(perm.roles, role.name) ? (
                              ...
                              <Lock size={16} className="text-on-surface-variant mx-auto" />
                            )}
                          </td>
                        ))}
                      </tr>
